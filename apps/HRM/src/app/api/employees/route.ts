...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Tạo tài khoản cho nhân viên (luôn tạo để họ có thể đăng nhập bằng Mã nhân viên)
    const loginEmail = data.companyEmail || `${employeeCode}@podsoftware.com.vn`
    const defaultPassword = employeeCode
    ...
    try {
      ...
      if (!existingUser) {
        ...
        if (process.env.NODE_ENV === "development") {
          console.log(`[DEV] User created: ${loginEmail} / ${defaultPassword}`)
        }
