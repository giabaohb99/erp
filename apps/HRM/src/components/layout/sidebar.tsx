  ...
  return (
    <>
      ...
      >
        ...
        >
          {!collapsed && <div className="font-semibold text-sm">POD HRM</div>}
          <button
          ...
          {mainGroups.map((group, gi) => (
            <div key={gi}>
              {group.label && !collapsed && (
                <div className="text-[10px] uppercase tracking-wider text-white/40 font-bold px-3 mt-5 mb-2 flex items-center gap-2">
                  {group.icon && iconMap[group.icon] && (
                    <span>
                      {(() => {
                        const GroupIcon = iconMap[group.icon];
                        return <GroupIcon className="h-3 w-3 opacity-50" />;
                      })()}
                    </span>
                  )}
