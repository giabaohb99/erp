...
describe('deliverWebhook (async)', () => {
  ...
  it('sends correct headers including HMAC signature', async () => {
    ...
    expect(fetchOptions.headers['X-BaoERP-Event']).toBe('order.created')
    expect(fetchOptions.headers['X-BaoERP-Signature']).toMatch(/^sha256=[a-f0-9]{64}$/)
    expect(fetchOptions.headers['X-BaoERP-Delivery']).toBeDefined()
  })
