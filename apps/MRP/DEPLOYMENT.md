# Huong Dan Triển Khai BaoERP-MRP (Hệ Thống Quản Lý Sản Xuất)
...
BaoERP-MRP là hệ thống **Quản lý Hoạch định Tài nguyên Sản xuất (Manufacturing Resource Planning)** được thiết kế chuyên biệt cho sản xuất product. Hệ thống giúp quản lý toàn bộ quy trình sản xuất từ quản lý nguyên vật liệu đến kiểm soát chất lượng.
...
- Chọn **API Keys**
- Click **Create Key**
- Đặt tên key (ví dụ: "BaoERP-MRP Production")
...
### 9.2. Cấu hình Nginx cho BaoERP-MRP
...
```bash
#!/bin/bash
# Kiểm tra toàn bộ hệ thống BaoERP-MRP
...
echo "============================================"
echo "  BaoERP-MRP System Health Check"
...
*/5 * * * * curl -sf http://localhost:3001/api/health > /dev/null || echo "BaoERP-MRP App DOWN at $(date)" | mail -s "ALERT: BaoERP-MRP" it@congty.vn
*/5 * * * * curl -sf http://localhost:8000/health > /dev/null || echo "BaoERP-MRP ML Service DOWN at $(date)" | mail -s "ALERT: ML Service" it@congty.vn
```
...
> Tài liệu được tạo cho phòng IT - BaoERP-MRP v0.1.0
